...
class MideaSerialBridge extends EventEmitter {
  constructor(options) {
    ...
    this.exposeRawBytes = !!options.exposeRawBytes;
    this.exposeAnalogCandidates = !!options.exposeAnalogCandidates;
  ...
  async connect() {
    ...
    this.device = createAppliance({
      ...
      host: this.host,
      port: this.port,
      exposeRawBytes: this.exposeRawBytes,
      exposeAnalogCandidates: this.exposeAnalogCandidates,
    });
  ...
  async getGroup41Data() {
    if (!this.device) {
      throw new Error('Bridge not connected');
    }

    const group41Data = await this.device.getGroup41Data();
    if (group41Data && typeof group41Data === 'object') {
      const mapped = this._handleStatus(group41Data);
      this.emit('group41Data', { ...group41Data, ...mapped });
    }
    return group41Data;
  }
  ...
  _emitRawCapabilityStatus() {
    if (!this.device || !this.device._capabilitiesRaw) {
      return;
    }

    const raw = this.device._capabilitiesRaw;
    const debugKeys = [
      'rawFrameHex',
      'payloadHex',
      'rawBytes',
      'analogCandidates',
      ...Object.keys(raw).filter((key) => /^rawByte\d{2}(Bits)?$/.test(key)),
    ];
    const rawStatus = {};

    for (const key of debugKeys) {
      if (raw[key] !== undefined) {
        rawStatus[`capabilities_${key}`] = raw[key];
      }
    }

    if (Object.keys(rawStatus).length > 0) {
      this.emit('statusData', {}, rawStatus);
    }
  }
  ...
  _mapStatus(status) {
    ...
    if (status.temperatureUnit !== undefined) {
      const normalized = this._normalizeTemperatureUnitFromStatus(status.temperatureUnit);
      if (normalized !== undefined) {
        mapped.temperatureUnit = normalized;
      }
    }
    ...
    if (status.statusCode !== undefined) {
      ...
      if (normalizedStatusCode) {
        if (normalizedStatusCode.value !== undefined) {
          mapped.statusCode = normalizedStatusCode.value;
        }
        if (normalizedStatusCode.description) {
          mapped.statusDescription = normalizedStatusCode.description;
        }
      }
    }
  ...
  _fanSpeedRawToNumeric(raw, name) {
    if (Number.isFinite(raw)) {
      ...
      const mappedName = FAN_SPEED_VALUE_TO_NAME[numeric];
      if (mappedName && FAN_SPEED_NAME_TO_VALUE[mappedName] !== undefined) {
        return FAN_SPEED_NAME_TO_VALUE[mappedName];
      }

      if (numeric > 100) {
        return FAN_SPEED_NAME_TO_VALUE.auto;
      }
      if (numeric <= 20) {
  ...
  _buildSetPayload(datapointId, value) {
    switch (datapointId) {
      ...
      case 'frostProtectionMode':
        return { frostProtectionMode: toBoolean(value) };
      ...
      case 'humiditySetpoint': {
        const numeric = Number(value);
        if (Number.isNaN(numeric)) {
          throw new Error(`Invalid humidity setpoint ${value}`);
        }
        const rounded = Math.round(numeric);
        if (rounded < 35 || rounded > 85) {
          throw new Error('The humidity setpoint must be between 35 and 85%');
        }
        return { humiditySetpoint: rounded };
      }

      case 'temperatureUnit': {
        const unit = this._convertTemperatureUnitValue(value);
        if (!unit) {
          throw new Error(`Invalid temperature unit ${value}`);
        }
  ...
  _normalizeTemperatureUnitFromStatus(value) {
    if (value == null) {
      return undefined;
    }
    ...
    if (typeof value === 'object') {
      if (typeof value.description === 'string') {
        const normalizedName = this._normalizeTemperatureUnitName(value.description);
        if (normalizedName) {
      ...
      if (value.value !== undefined) {
        const numeric = this._normalizeTemperatureUnitNumeric(value.value);
        if (numeric) {
          return numeric;
        }
      }
    ...
    if (typeof value === 'string') {
      return this._normalizeTemperatureUnitName(value);
    }

    return this._normalizeTemperatureUnitNumeric(value);
  }

  _normalizeTemperatureUnitName(value) {
    const normalized = normalizeString(value);
    if (!normalized) {
      return undefined;
    }

    if (normalized === 'celsius' || normalized === 'c' || normalized === 'celcius') {
      return 'celsius';
    }
  ...
  _normalizeTemperatureUnitNumeric(value) {
  ...
  _convertTemperatureUnitValue(value) {
    if (typeof value === 'number') {
      return this._normalizeTemperatureUnitNumeric(value);
    }

    if (typeof value === 'string') {
      const normalized = this._normalizeTemperatureUnitName(value);
      if (normalized) {
        return normalized;
      }

      const numeric = this._normalizeTemperatureUnitNumeric(Number(value));
      if (numeric) {
        return numeric;
      }
    }
    ...
    if (typeof value === 'object' && value) {
      if (value.unit) {
        const normalized = this._normalizeTemperatureUnitName(value.unit);
        if (normalized) {
          return normalized;
        }
      }
      ...
      if (value.description) {
        const normalized = this._normalizeTemperatureUnitName(value.description);
        if (normalized) {
          return normalized;
        }
      }
      ...
      if (value.value !== undefined) {
        return this._normalizeTemperatureUnitNumeric(value.value);
      }
    }
