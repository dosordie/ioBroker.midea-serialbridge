...
module.exports = class extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      self
        ...
        .then(async (response) => {
          ...
          let parsedData = self._parseResponse(response);
  ...
  getGroup41Data(retry = 0) {
    const self = this;

    logger.silly('AC.getGroup41Data: Entering');

    const cmd = createGroupDataCommandByByte(0x41);

    return new Promise((resolve, reject) => {
      self
        ._request(cmd, 'getGroup41Data', retry)
        .then((response) => {
          if (response[10] !== 0xc1) {
            return reject(new Error('Invalid response'));
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const property in properties) {
        switch (property) {
          ...
          case 'ecoMode':
            logger.debug(`AC.setStatus: Set eco mode to ${properties[property] === true}`);
